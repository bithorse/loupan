...
export default {
  ...
  methods: {
      ...
      this.$http({
		...
		if(json.tanzhushouji!=''&&json.tanzhushouji){
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/shichangguanliyuan/guanlizhanghao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
    ...
    guanlizhanghaoChange () {
      this.$http({
        url: `follow/shichangguanliyuan/guanlizhanghao?columnValue=`+ this.ruleForm.guanlizhanghao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              ...
              if(data.data.guanlidianhua){
                this.ruleForm.guanlidianhua = data.data.guanlidianhua
              }
        } else {
          this.$message.error(data.msg);
        }
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `tousuyijian/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.tousuyijianCrossAddOrUpdateFlag = false;
						 this.parent.search();
