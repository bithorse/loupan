<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="街道区域" prop="jiedaoquyu">
          <el-select :disabled="ro.jiedaoquyu" v-model="ruleForm.jiedaoquyu" placeholder="请选择街道区域">
            <el-option
                v-for="(item,index) in jiedaoquyuOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
...
export default {
  data() {
	...
	jiedaoquyu : false,
      ...
      rules: {
          ...
          jiedaoquyu: [
                { required: true, message: '街道区域不能为空', trigger: 'blur' },
          ],
      }
    };
  },
  ...
  methods: {
		...
		if(this.$storage.get("role")!="管理员") {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              ...
              if (data && data.code === 0) {
                this.jiedaoquyuOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    info(id) {
      this.$http({
        url: `shichangguanliyuan/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `shichangguanliyuan/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.shichangguanliyuanCrossAddOrUpdateFlag = false;
						 this.parent.search();
